...
export class MediasoupService {
  ...
  /**
   * delete the peer of cossumer has left the room
   * @param id the id of consumer has left the room
   */
  public async deletePeer(id: string){
    this.consumersVideoStream.delete(id);
    this.consumersVideo.delete(id);
    this.consumersAudioStream.delete(id);
    this.consumersAudio.delete(id);
  }
   ...
   */
  public async producerVideoStart(ratio = 1): Promise<void> {
    try {
      if (this.mediasoupDevice.canProduce('video')) {
        const videoStream = await navigator.mediaDevices.getUserMedia({ video: { width: 500 * ratio , height: 500 }});
